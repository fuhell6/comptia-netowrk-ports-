...
def show_question():
    ...
    feedback_var.set("")
    score_var.set(f"✅ Correct: {correct_count} ❌ Wrong: {wrong_count}")
...
def check_tf_answer(answer):
    ...
    if answer == is_secure:
        ...
        wrong_count += 1
        feedback_var.set(f"❌❌ Incorrect. Port {port} is {'secure' if is_secure else 'not secure'}.")
        feedback_label.config(fg="red")
...
def check_mcq_answer(selected):
    ...
    if selected == correct:
        ...
        wrong_count += 1
        feedback_var.set(f"❌❌ Incorrect. Port {port} is {ports_info[port][1]} — {ports_info[port][2]}.")
        feedback_label.config(fg="red")
...
def show_summary():
    question_var.set("🎉 Quiz Complete!")
    for widget in quiz_frame.winfo_children():
        widget.pack_forget()
...
reset_button = tk.Button(quiz_frame, text="🔁 Reset 🔁", width=20, command=reset_quiz)
reset_button.pack(pady=5)

# Dark mode toggle as icon button
toggle_button = tk.Button(top_frame, text="☀️", width=4, command=toggle_dark_mode)
toggle_button.pack()
...
#toggle_btn = tk.Button(top_frame, text="🌙 Toggle Dark Mode b🌙 ", width=24, command=toggle_dark_mode)
#toggle_btn.pack()

back_btn = tk.Button(bottom_frame, text="⬅ Back ⬅ ", width=20, height=4,  command=previous_question)
back_btn.pack(side=tk.LEFT, padx=55)

next_btn = tk.Button(bottom_frame, text="➡ Next ➡", width=20, height=4, command=next_question)
